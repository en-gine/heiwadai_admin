...
export type SelectPrefProps = Omit<SelectProps, "onValueChange"> & {
  onSelectRegion?: (keys: number[]) => void
  onValueChange(key: number): void
}

export const SelectPref = (props: SelectPrefProps) => {
  const handleLabelClick = (keys: number[]) => {
    if (props.onSelectRegion) {
      props.onSelectRegion(keys)
    }
  }
  return (
    <Select
      ...
      onValueChange={(item: string) => {
        if (props.onValueChange) {
          props.onValueChange(parseInt(item, 10))
        }
      }}
    >
      <SelectTrigger>
      ...
      <SelectContent>
        <SelectGroup className="overflow-y-auto max-h-[25rem]">
          ...
          <SelectGroup>
            <SelectLabel
              onClick={() => handleLabelClick([Prefecture.Hokkaido])}
            >
              北海道
            </SelectLabel>
